...
<div class=" antialiased font-sans text-gray-900">
    ...
    <div class="px-20 py-20">
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            ...
            <h1 class="font-semibold text-3xl text-indigo-700 mb-8 text-center ">Bảng Thiết Bị</h1>
                    <form class="flex justify-end pb-2" action="{{ route('dsthietbi') }}" method="GET">
                ...
                <table class="w-full whitespace-no-wrap">
                    ...
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3">ID Thiết Bị</th>
                            <th class="px-4 py-3">ID Phòng</th>
                            <th class="px-4 py-3">Thiết Bị</th>
                            <th class="px-4 py-3">Tình Trạng</th>
                            <th class="px-4 py-3">Tính Năng</th>
                    ...
                    @if(session('search_keyword'))
                        @if(count($Devis) == 0)
                            <tr>
                                <td colspan="7" class="text-center text-red-600 p-2 text-2xl">Không Tồn Tại Thiết Bị Phòng Thí Nghiệm {{session('search_keyword')}} </td>
                            </tr>
                        @else
                            @foreach ($Devis as $de)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3">{{$de->idthietbi}}</td>
                                <td class="px-4 py-3">{{$de->idphong}}</td>
                                <td class="px-4 py-3">{{$de->tenthietbi}}</td>
                                <td class="px-4 py-3">{{$de->tinhtrang}}</td>
                                <td class="px-4 py-3">{{$de->tinhnang}}</td>
                                <td class="px-4 py-3">
                                    <a onclick="deletetb('{{ $de->id}}');" class="flex justify-center items-center">
                                ...
                                <td class="px-4 py-3">
                                    <a href="{{route('capnhat', ['id'=> $de->idthietbi])}}"
    ...
    function submitForm(id) {
        ...
        form.action = "{{ route('deletetb', ['id' => ':id']) }}";
